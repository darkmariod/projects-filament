...
class LabelPdfService
{
    ...
    protected function buildLabelHtml(Label $label): string
    {
        ...
        // Brand logo: product image → category logo → Paraíso logo → text fallback
        $logoHtml = '<div class="muy-grande">PARAISO</div>';

        // 1. Try product image
        if (!empty($product->image)) {
            $imgPath = storage_path('app/public/' . $product->image);
            if (is_file($imgPath)) {
                $logoBase64 = base64_encode(file_get_contents($imgPath));
                $mime = mime_content_type($imgPath);
                $logoHtml = "<img src=\"data:{$mime};base64,{$logoBase64}\" style=\"width:110px;\">";
            }
        }

        // 2. Try category logo
        if ($logoHtml === '<div class="muy-grande">PARAISO</div>') {
            $catLogo = $product->productModel->category->logo ?? null;
            if ($catLogo) {
                $catPath = storage_path('app/public/' . $catLogo);
                if (is_file($catPath)) {
                    $logoBase64 = base64_encode(file_get_contents($catPath));
                    $mime = mime_content_type($catPath);
                    $logoHtml = "<img src=\"data:{$mime};base64,{$logoBase64}\" style=\"width:110px;\">";
                }
            }
        }

        // 3. Try Paraíso logo fallback
        if ($logoHtml === '<div class="muy-grande">PARAISO</div>') {
