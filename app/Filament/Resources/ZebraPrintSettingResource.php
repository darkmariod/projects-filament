...
use App\Services\ZebraZplRenderer;
                ...
                Action::make('test_print')
                    ...
                    ->form([
                        Forms\Components\Select::make('product_id')
                            ...
                            ->preload()
                            ->nullable()
                            ->helperText('Selecciona un producto para probar con su logo. Déjalo vacío para prueba básica.'),
                    ])
                    ->requiresConfirmation()
                    ->modalHeading('Test de impresión Zebra')
                    ->modalDescription('Se enviará una etiqueta de prueba. Si seleccionas un producto, se usará su logo.')
                    ->action(function (ZebraPrintSetting $record, array $data) {
                        $product = !empty($data['product_id'])
                            ? Product::find($data['product_id'])
                            : null;

                        $renderer = new ZebraZplRenderer($record);
                        $testZpl = $renderer->renderTestLabel($product);
