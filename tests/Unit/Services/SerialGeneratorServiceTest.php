...
class SerialGeneratorServiceTest extends TestCase
{
    ...
    /** @test */
    public function it_sets_barcode_equal_to_serial_in_generateLabelsForBatch(): void
    {
        $batch = $this->createBatch(quantity: 2, productCode: 'BAR01', date: '2026-05-27');
        ...
        foreach ($labels as $label) {
            $this->assertSame($label->serial, $label->barcode, 'Label barcode must equal serial');
        }
    }

    /** @test */
    public function it_uses_serial_as_barcode_not_product_barcode(): void
    {
        $batch = $this->createBatch(quantity: 1, productCode: 'BCDIFF', date: '2026-05-27');

        // Override the product barcode to something distinct
        $batch->product->update(['barcode' => '0000000000001']);
        $batch->refresh();
        ...
        $this->assertNotSame('0000000000001', $label->barcode, 'Barcode must NOT be the product barcode');
        $this->assertSame($label->serial, $label->barcode, 'Barcode must equal the generated serial');
    }
